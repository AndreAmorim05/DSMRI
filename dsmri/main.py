...
from dsmri.features import Extract_features
...
def single_subject_info(input_dir):
    if not input_dir.endswith(".nii.gz"):
        raise ValueError("The file inputed is not on an expected format (.nii.gz)")

    subject = [
        os.path.basename(input_dir)[: os.path.basename(input_dir).index(".")],
    ]
    return [input_dir], subject
...
def subject_info(input_dir):
    ...
    subjects = [
        os.path.basename(scan)[: os.path.basename(scan).index(".")] for scan in files
    ]
...
def saveThumbnails(v, output, slice_gap, central_size):
    ...
    for i in range(start, finish, slice_gap):
        cnt = cnt + 1
        plt.imsave(
            output + os.sep + v[1] + os.sep + v[1] + "(%d).png" % int(i + 1),
            scipy.ndimage.rotate(v[0][i], 270),
            cmap=cm.Greys_r,
        )
...
def worker_callback(s, fname_outdir):
    ...
    if nfiledone == 0:
        csv_report = open(
            fname_outdir + os.sep + "results" + ".tsv", overwrite_flag, buffering=1
        )
        first = True
...
def dsmri_run(
    ...
    headers.append(f"outdir:\t{os.path.realpath(output_path)}")
    files, subject_names = single_subject_info(filepath)

    if len(files) == 0:
        raise RuntimeError(
            "The input folder is empty or includes unsupported file formats!"
        )

    for l, k in enumerate(files):
        v = volume(k, subject_names[l])
        saveThumbnails(v, output_path, slice_gap, central_size)
        s = Extract_features(output_path, v, l + 1, slice_gap, central_size)
        worker_callback(s, output_path)

    address = os.path.join(output_path, "results.tsv")

    if len(subject_names) < perplexity:
        print(
            "Insufficient data for t-SNE and UMAP. The UMAP and t-SNE methods require the total samples ",
    ...
    print(
        "The features are saved in the {} file. ".format(
            output_path + os.sep + "features.csv"
        )
    )
    print(
        "DSMRI program took",
...
def main():
    ...
    parser.add_argument(
        "-b", help="number of gap in consecutive slices", default=5, type=int
    )
    ...
    for l, k in enumerate(files):
        ...
        s = Extract_features(output_dir, v, l + 1, slice_gap, central_size)
    ...
    print(
        "The features are saved in the {} file. ".format(
            output_dir + os.sep + "features.csv"
        )
    )
    print(
        "DSMRI program took",
...
if __name__ == "__main__":
    subject = "/home/andre/gitworkspace/ENACOM/FINEP/DATASETS/ID44_T1w_conformed/OAS30027/sub-OAS30027_ses-d2394_T1w.nii.gz"
    output_dir = "/home/andre/gitworkspace/ENACOM/FINEP/DSMRI/Results/outputs"
    dsmri_run(filepath=subject, output_path=output_dir)
